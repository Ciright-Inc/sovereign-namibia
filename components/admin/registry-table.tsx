            ...
            {records.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-[#6b7280]">
                  No records indexed yet. Use Data Imports or create a new record.
                </td>
              </tr>
            ) : (
              records.map((record) => (
                <tr key={record.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                  <td className="px-4 py-3 font-mono text-xs text-emerald-400/90">
                    <Link href={`/admin/registry/${record.id}`} className="hover:underline">{record.registry_id}</Link>
                  </td>
                  <td className="px-4 py-3">
                    <Link href={`/admin/registry/${record.id}`} className="font-medium text-white hover:underline">{record.name}</Link>
                  ...
                  <td className="px-4 py-3">
                    <span className="rounded border border-white/10 px-2 py-0.5 text-xs capitalize text-[#d1d5db]">{record.status}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`rounded px-2 py-0.5 text-xs capitalize ${record.verification_status === "verified" ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}>
